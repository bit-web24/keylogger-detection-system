...
class LinuxKeyloggerDetector:
    def __init__(self):
        self.suspicious_keywords = [
            'keylog', 'keycap', 'keystroke', 'keyrecord', 'logkeys',
            'keysniff', 'keymonitor', 'inputlog', 'screenlog', 'spyware',
            'logger', 'capture', 'xinput', 'xev', 'xdotool', 'hook'
        ]
    ...
    def calculate_threat_score(self, proc_info):
        ...
        if '/.' in exe_path or exe_path.startswith('/tmp') or exe_path.startswith('/dev/shm'):
            score += 20
    ...
    def check_systemd_services(self):
        ...
        try:
            ...
            for service in services:
                ...
                if any(keyword in service_lower for keyword in self.suspicious_keywords):
                    parts = service.split()
                    if parts:
                        suspicious.append({
                            'name': parts[0],
                            'status': 'active' if 'active' in service_lower else 'inactive',
            ...
            try:
                for filename in os.listdir(path):
                    ...
                    if os.path.isfile(filepath):
                        ...
                        if any(keyword in content or keyword in filename.lower() 
                               for keyword in self.suspicious_keywords):
                            suspicious.append({
                                'name': filename,
                                'path': filepath,
        ...
        try:
            ...
            if result.returncode == 0:
                for line in result.stdout.split('\n'):
                    ...
                    if any(keyword in line_lower for keyword in self.suspicious_keywords):
                        suspicious.append({
                            'name': 'Crontab Entry',
                            'path': line.strip(),
        ...
        for conn in psutil.net_connections(kind='inet'):
            if conn.status == 'ESTABLISHED':
                try:
                    ...
                    if any(keyword in proc_name for keyword in self.suspicious_keywords):
...
class ModernLinuxKeyloggerGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Advanced Linux Anti-Keylogger Protection Suite")
    ...
    def setup_styles(self):
        ...
        style.configure('Primary.TButton', background=self.colors['primary'], 
                       foreground='#ffffff', font=('Ubuntu', 10, 'bold'),
                       borderwidth=0, focuscolor='none', padding=10)
        style.map('Primary.TButton', background=[('active', '#1f6feb')])
        ...
        style.configure('TNotebook.Tab', background=self.colors['surface'], 
                       ...
                       font=('Ubuntu', 10, 'bold'))
        style.map('TNotebook.Tab', background=[('selected', self.colors['surface_light'])],
        ...
        style.configure('Treeview', background=self.colors['surface'], 
                       ...
                       borderwidth=0, font=('Ubuntu Mono', 9))
        style.configure('Treeview.Heading', background=self.colors['surface_light'], 
                       foreground=self.colors['primary'], font=('Ubuntu', 10, 'bold'),
    ...
    def create_gui(self):
        ...
        title_label = ttk.Label(header_frame, text="🛡️ Linux Keylogger Detection Suite", 
                               style='Title.TLabel')
    ...
    def create_process_tab(self, parent):
        info_label = ttk.Label(parent, text="Detected processes with suspicious behavior patterns",
                              style='Header.TLabel')
        info_label.pack(anchor='w', padx=10, pady=5)
        
        columns = ("PID", "Name", "User", "Threat", "Score", "Command")
        self.proc_tree = ttk.Treeview(parent, columns=columns, show="headings", height=18)
        ...
        scrollbar = ttk.Scrollbar(parent, orient='vertical', command=self.proc_tree.yview)
    ...
    def create_startup_tab(self, parent):
        info_label = ttk.Label(parent, text="Suspicious autostart entries and systemd services",
                              style='Header.TLabel')
        info_label.pack(anchor='w', padx=10, pady=5)
        
        columns = ("Type", "Name", "Status", "Path")
        self.startup_tree = ttk.Treeview(parent, columns=columns, show="headings", height=18)
        ...
        scrollbar = ttk.Scrollbar(parent, orient='vertical', command=self.startup_tree.yview)
        self.startup_tree.configure(yscrollcommand=scrollbar.set)
        
        self.startup_tree.pack(side='left', fill='both', expand=True, padx=(10, 0), pady=5)
        scrollbar.pack(side='right', fill='y', padx=(0, 10), pady=5)
        
    def create_network_tab(self, parent):
        info_label = ttk.Label(parent, text="Active network connections from suspicious processes",
                              style='Header.TLabel')
        info_label.pack(anchor='w', padx=10, pady=5)
        
        columns = ("PID", "Process", "Local", "Remote")
        self.network_tree = ttk.Treeview(parent, columns=columns, show="headings", height=18)
        ...
        self.network_tree.pack(side='left', fill='both', expand=True, padx=(10, 0), pady=5)
        scrollbar.pack(side='right', fill='y', padx=(0, 10), pady=5)
        
    def create_input_tab(self, parent):
        info_label = ttk.Label(parent, text="Processes accessing keyboard/input devices (/dev/input)",
                              style='Header.TLabel')
        info_label.pack(anchor='w', padx=10, pady=5)
        
        columns = ("PID", "Process", "Device", "Executable")
        self.input_tree = ttk.Treeview(parent, columns=columns, show="headings", height=18)
        ...
        scrollbar = ttk.Scrollbar(parent, orient='vertical', command=self.input_tree.yview)
        self.input_tree.configure(yscrollcommand=scrollbar.set)
        
        self.input_tree.pack(side='left', fill='both', expand=True, padx=(10, 0), pady=5)
        scrollbar.pack(side='right', fill='y', padx=(0, 10), pady=5)
        
    def create_monitor_tab(self, parent):
        info_label = ttk.Label(parent, text="Real-time monitoring of suspicious system activity",
                              style='Header.TLabel')
        info_label.pack(anchor='w', padx=10, pady=5)
        
        self.monitor_text = tk.Text(parent, bg=self.colors['surface'], 
                                   fg=self.colors['fg'], font=('Ubuntu Mono', 9),
                                   height=25, wrap='word', insertbackground=self.colors['primary'])
        self.monitor_text.pack(fill='both', expand=True, padx=10, pady=5)
        
        scrollbar = ttk.Scrollbar(parent, orient='vertical', command=self.monitor_text.yview)
    ...
    def create_control_panel(self):
        ...
        self.scan_btn = ttk.Button(control_frame, text="🔍 Full System Scan", 
                                   command=self.full_scan, style='Primary.TButton')
        self.scan_btn.pack(side='left', padx=5)
        
        self.terminate_btn = ttk.Button(control_frame, text="⚠️ Terminate Process", 
                                       command=self.terminate_selected, style='Danger.TButton')
        self.terminate_btn.pack(side='left', padx=5)
        
        self.monitor_btn = ttk.Button(control_frame, text="▶️ Start Monitoring", 
                                     command=self.toggle_monitoring, style='Primary.TButton')
        self.monitor_btn.pack(side='left', padx=5)
        
        self.save_btn = ttk.Button(control_frame, text="💾 Save Report", 
                                  command=self.save_report, style='Primary.TButton')
        self.save_btn.pack(side='left', padx=5)
        
        self.threat_label = ttk.Label(control_frame, text="Threats: 0", 
                                     font=('Ubuntu', 12, 'bold'),
                                     foreground=self.colors['success'])
        self.threat_label.pack(side='right', padx=10)
        
    def full_scan(self):
        self.status_label.config(text="● Scanning System...", foreground=self.colors['warning'])
        ...
        def scan_thread():
            ...
            self.root.after(0, lambda: self.log_monitor("[SYSTEM] Starting full system scan...", 'info'))
            
            # Scan processes
            self.root.after(0, lambda: self.log_monitor("[SCAN] Checking running processes...", 'info'))
            ...
            # Scan startup
            self.root.after(0, lambda: self.log_monitor("[SCAN] Checking startup entries...", 'info'))
            ...
            self.root.after(0, lambda: self.log_monitor("[SCAN] Checking systemd services...", 'info'))
            ...
            # Scan network
            self.root.after(0, lambda: self.log_monitor("[SCAN] Checking network connections...", 'info'))
            ...
            # Scan input devices
            self.root.after(0, lambda: self.log_monitor("[SCAN] Checking input device access...", 'info'))
            ...
            total_threats = (len(self.suspicious_procs) + len(self.suspicious_startup) + 
                           len(self.suspicious_connections) + len(self.suspicious_services) +
    ...
    def update_after_scan(self, total_threats):
        ...
        if total_threats > 0:
            self.status_label.config(text="⚠️ Threats Detected!", foreground=self.colors['danger'])
            self.threat_label.config(text=f"Threats: {total_threats}", 
                                   foreground=self.colors['danger'])
            self.log_monitor(f"[ALERT] Scan complete: {total_threats} potential threats found!", 'alert')
        else:
            self.status_label.config(text="✓ System Clean", foreground=self.colors['success'])
    ...
    def terminate_selected(self):
        ...
        if not selected:
            messagebox.showinfo("No Selection", "Please select a process to terminate from the Processes tab.")
            return
        ...
        for item in selected:
            ...
            if messagebox.askyesno("Confirm Termination", 
                ...
                try:
                    ...
                    self.log_monitor(f"[TERMINATED] Successfully killed PID {pid} ({name})", 'success')
                    messagebox.showinfo("Success", f"Process {pid} terminated successfully.")
                except psutil.TimeoutExpired:
                    try:
                        proc.kill()
                        self.log_monitor(f"[KILLED] Force killed PID {pid} ({name})", 'warning')
                    except:
                        pass
                except PermissionError:
                    messagebox.showerror("Permission Denied", 
                                       f"Cannot terminate PID {pid}. Try running with sudo.")
                    self.log_monitor(f"[ERROR] Permission denied to terminate PID {pid}", 'alert')
                except Exception as e:
                    messagebox.showerror("Error", f"Failed to terminate: {str(e)}")
                    self.log_monitor(f"[ERROR] Failed to terminate PID {pid}: {str(e)}", 'alert')
    ...
    def save_report(self):
        ...
        try:
            with open(file_path, 'w') as f:
                f.write("=" * 90 + "\n")
                f.write("LINUX ANTI-KEYLOGGER DETECTION REPORT\n")
                ...
                if self.suspicious_procs:
                    for proc in self.suspicious_procs:
                        ...
                        f.write(f"Threat Level: {proc['threat_level']} | Score: {proc['threat_score']}%\n")
                ...
                if self.suspicious_connections:
                    for conn in self.suspicious_connections:
                        ...
                        f.write(f"Local: {conn['local']} -> Remote: {conn['remote']}\n\n")
                else:
                ...
                f.write("-" * 90 + "\n")
                f.write("1. Review all flagged processes carefully before taking action\n")
                f.write("2. Check /var/log/auth.log and /var/log/syslog for suspicious activity\n")
                ...
                f.write("5. Consider using AppArmor or SELinux for additional protection\n")
...
if __name__ == "__main__":
    ...
    app.log_monitor("Linux Anti-Keylogger Protection Suite v2.0", 'info')
    app.log_monitor("Protecting your system from unauthorized input monitoring", 'info')
    app.log_monitor("=" * 60, 'info')
    app.log_monitor("[INFO] Ready to scan. Click 'Full System Scan' to begin.", 'success')
